...
from ltx_editor.core.document import Document, Section, ListType, GraphType
...
class FitxCompiler:
    ...
    def compile(self, src: str) -> str:
        ...
        for tag, props, content in self._parse(src):
            ...
            if tag in self.tokens:
                ...
                print(f"Warning: Unrecognized token '{tag}' found.")
        self.converter=JsonToHtmlConverter(self.document.get_context())
        print(self.document.get_context())
    ...
    def handle_list(self, props, element_data):
        ...
        if props and props[0] == "ORDERED":
            ...
            self.document.get_last_section().addText(f"Error: '{props[0] if props else 'None'}' is not a valid list type. Use ORDERED or UNORDERED.")
    ...
    def handle_table(self, props, element_data):
        try:
            self.document.get_last_section().addTable(headers=props,data=ast.literal_eval(re.sub(r'(\b[a-zA-Z\s-]+)\b', r"'\1'", element_data)))
...
class JsonToHtmlConverter:
    """
    A class to convert a structured JSON object into a single HTML file.
    This version includes global settings, visual page separation, and full support for all content types.
    It now correctly handles manual page breaks without creating blank pages.
    The page-specific orientation feature has been removed for a more robust solution.
    """

    def __init__(self, json_data):
        """
        Initializes the converter with the JSON data to be processed.
        It also extracts and validates the global settings.
        """
        self.data = json_data
