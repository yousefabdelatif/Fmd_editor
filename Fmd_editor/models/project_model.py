...
class ProjectModel:
    ...
    def save_data_to_disk(self, data: str) -> int:
        ...
        if not self.__basePath:
            ...
            raise ValueError("Cannot save data: project base path is not set.")

        try:
            with open(self.__basePath / Path("src/data.fmd"), "w") as file:
                self.__data = data
                file.write(data)
                return 0
        except IOError as e:
    ...
    def load_from_disk(self, path: Path):
        ...
        try:
            with open(self.__basePath / "src" / "data.fmd", "r") as file:
                self.__data = file.read()
        except FileNotFoundError:
            raise FileNotFoundError(f"Source file not found at {self.__basePath / 'src/data.fmd'}")
    ...
    def create_new(self, name: str, path: Path, template_version: str):
        ...
        try:
            ...
            with open(project_path / "src/data.fmd", "w") as data_file:
